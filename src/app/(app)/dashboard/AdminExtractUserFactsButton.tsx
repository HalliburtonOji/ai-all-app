...
import { manuallyExtractUserFacts } from "./user-fact-actions";

export function AdminExtractUserFactsButton() {
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      const result = await manuallyExtractUserFacts();
      if ("error" in result) {
  ...
  return (
    <div className="mb-3 flex items-center justify-between gap-3 rounded-md border border-zinc-300 bg-zinc-50 px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900">
      <div className="flex flex-col gap-0.5">
        <span className="font-medium text-zinc-700 dark:text-zinc-300">
          Admin: extract user-level facts
        </span>
        {feedback && (
      ...
      <button
        ...
        onClick={handleClick}
        disabled={isPending}
        data-extract-user-facts-button="true"
      ...
      >
        {isPending ? "Running…" : "Run user extraction now"}
      </button>
    </div>
  );
}
